...
class List
  ...
  def replaceSpecificPatternInString(string, pattern, replacementPattern)
    ...
    fail 'Pattern not removed from list: ' + pattern if string.nil?
  ...
  def removeFootnotes(contents)
    (1..49).each do |footnoteNum|
      ...
      unless footnoteRemoved
        fail 'Footnote #' + footnoteNumString + ' not found and not removed'
      end
    end
    contents
  end
  ...
  def checkNoImgTags(contents)
    ...
    if scan.length > 0
      puts scan
      fail 'Forbidden image tags found'
    end
  ...
  def printPrisoner(prisoner)
    ...
    puts prisoner if prisonerIdsToPrint.include? prisoner.getId
  end
      ...
      @prisonerTypes.each do |prisonerType|
        prisonerType.getPrisoners.each do |prisoner|
          ...
          prisoner.getCharges.each do |charge|
            if chargeNumbers == ''
              chargeNumbers += charge.getNumber
            else
  ...
  def outputPlacesOfDetention(output_path)
    ...
    getPrisoners.each do |prisoner|
      ...
      if placeOfDetention != 'Not Listed'
        unless uniquePrisons.include? placeOfDetention
    ...
    getPrisoners.each do |prisoner|
      ...
      charges.each do |charge|
        ...
        uniqueArticles.each do |uniqueArticle|
          if uniqueArticle.getNumber == charge.getNumber && uniqueArticle.getCriminalCode == charge.getCriminalCode
            chargeNotStoredYet = false
          end
        end

        uniqueArticles.push(charge) if chargeNotStoredYet
      end
    end

    uniqueArticles = uniqueArticles.sort_by(&:getNumber)
