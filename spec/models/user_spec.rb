...
RSpec.describe User, :type => :model do
  ...
  describe "with email, password and role" do
    it "is valid" do
      content_manager_user = FactoryGirl.build(:user)
      expect { content_manager_user.save! }.not_to raise_error
    end
  end

  describe "with email, password and no role" do
    it "is not valid" do
      content_manager_user = FactoryGirl.build(:user, role: nil)
      expect { content_manager_user.save! }.to raise_error
    end
  end

  describe "with email, role and no password" do
    it "is not valid" do
      content_manager_user = FactoryGirl.build(:user, password: '')
      expect { content_manager_user.save! }.to raise_error
    end
  end
  ...
  describe "with password, role and no email" do
    it "is not valid" do
      content_manager_user = FactoryGirl.build(:user, email: '')
      expect { content_manager_user.save! }.to raise_error
    end
  end
end
